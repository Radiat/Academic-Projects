...
public class TilePuzzle {
  ...
  public boolean moveTile(int cellToMove) {
    ...
    int rowPos = 0;
    int colPos = 0;
    // This section is to determine the location of the tile.
    ...
    // This ugly part is to determine which compass direction the blank
    // space is at, and to swap their values if found.
    ...
    // If the move has occured, increment the number of moves.
    if (move) {
      numMoves++;
    }
    return move;
  ...
  public boolean equals(TilePuzzle puzzle) {
    ...
    //This is to determine if they have equal dimensions.
    boolean isEquals = rowLength == (otherGrid.length - 1) 
         && columnLength == (otherGrid[0].length - 1);
    //If they have equal dimensions, determine if each element is the same.
    if (isEquals) {
      for (int i = 0; i <= rowLength; i++) {
      for (int j = 0; j <= columnLength; j++) {
