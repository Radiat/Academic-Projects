...
public class TilePuzzleTester extends TestCase {
   ...
   */
    ...
    assertTrue(puzzle.equals(puzz2));
     ...
     /**
       * A slightly different test that focuses on moving a tile from the
       * corner, which is also a test of a side (or in this case, two sides).
       */
      public void testmoveTileCorner() {
        int[][] nums = {{-1, 2, 3}, {4, 5, 6}, {7, 8, 1}};
        TilePuzzle puzzle = new TilePuzzle(nums);
        assertTrue(puzzle.moveTile(4));
        int[][] other = {{4, 2, 3}, {-1, 5, 6}, {7, 8, 1}}; 
        TilePuzzle puzz2 = new TilePuzzle(other);
        assertTrue(puzzle.equals(puzz2));
      }
      
      /**
       * A simple yet tricky test that focuses on the 1XN formation, which 
       * in this case the 1 is the num. of columns (maybe 1XN was written
       * with columns times rows in mind?)
       */
      public void testmoveTileOneD() {
        int[][] nums = {{-1}, {2}, {3}, {1}};
        TilePuzzle puzzle = new TilePuzzle(nums);
        assertTrue(puzzle.moveTile(2));
        int[][] other = {{2}, {-1}, {3}, {1}};
